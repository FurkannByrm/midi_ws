...
#define left_ph 18  // Dir Motor left
#define left_en 13  // PWM left
#define left_vref 6  // Dir Motor 
...
void loop() {
  ...
  if (Serial.available())
  {
    ...
    else if(chr == 'p')
    {
      if(is_right_wheel_cmd && !is_right_wheel_forward)
      {
        ...
        digitalWrite(right_ph, HIGH - digitalRead(right_ph));
        is_right_wheel_forward = true;
      }
      else if(is_left_wheel_cmd && !is_left_wheel_forward)
      {
        // change the direction of the rotation
        digitalWrite(left_ph, HIGH - digitalRead(left_ph));
  ...
  if(current_millis - last_millis >= interval)
  {
    ...
    analogWrite(left_en, right_wheel_cmd);
    analogWrite(right_en, left_wheel_cmd);
  }
}
